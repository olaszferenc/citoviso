...
favikon (világos E4); őr `brand-mark-check` (4 negatív kontroll). ADR-0236. **Nyitott:** a számla-logó a TESZT
fiókban fent (szorosan vágott változat, próba: OV-2026-60) — az ÉLES fiókba a tulaj tölti fel, és döntés kell, hogy a
repóbeli margós PNG-t a szorosra cseréljük-e; a citoviso.com hero-illusztrációja is
E4 (`heroMarkSvg()`, fehér ív a cián gömbön). Nincs élesítve. Jegyzet: `_planning/memory/2026-09-26_logo_b_lockup_mindenhol.md`.
...
## Aktív feladat (legfrissebb szál, 2026-09-26 — Mobil · vendég-nézet 2: a motor + egy lead 19 stílusa)

**📱 A FK-009 ÁTADOTT NÉGY TÉTELE A MOCK-MOTORBAN — landolva (`origin/main` = `8082672a`, ADR-0237), NEM élesítve.**
① Masthead telefonon „C”: ≤720 px VAGY ≤500 px magas → tömör (62–128 px, volt 148–200), a link-sáv elmarad, ahol a sablon
rögzített Foglalás-sávot visel (`mastheadHtml({phoneBar:true})`), overlay léniák ki; artdeco + claymorphism ÚJ sávot kapott
(gomb `min-width:124` — a pirula-elhelyező 100 px alatt ráült, mérve 97 px-en). Asztalon pixelre változatlan. ② Google-fotó:
`responsiveGooglePhotos()` (render.ts): `srcset` 480/800/1200 `-rw` + `sizes`; háttér-hero `data-cit-bg` + `image-set`; a `src`
az eredeti; portál-fotó (hovamenjek, lake-balaton.com, apartman.hu) NEM méretezhető → érintetlen. ③ Sáv-felirat egy szerkezet
(`mobCtaStat`), transit főcím 32 px. ④ Brutalism marquee: mérve rendben (a 515/780 px a galéria volt, FK-010 javította).
Őrök: `guest-mobile-check` ②fejléc-blokk/⑤sáv-felirat (+2 negatív kontroll; a 120 px-es ültetés 149-et mért — 200 lett),
a Google map-embed saját `google is not defined` hibája mindkét mobil-őrben harmadik félként szűrve (2/30 piros terhelés
alatt, 0/2 egyedül — NEM bizonyított javítás, kimondva); ÚJ `photo-srcset-check`. Kontraktus:
`assets/design-refs/engine/name-masthead/phone/` (README = ami köt, plan.html, shots). **A tulaj UTÓLAG ítél** a kész felületen.
**② Alig-vár Tanya (Salföld, tanya-típus, `46f8b0ae…`) mind a 19 stílusa KÉSZ** — a fő fából, a konzol útján
(`generateEngineMock` + `template`), 19/19 ✓, $5,72 AI-költség, 18/19 tényhűség-FLAG → kurátor-sor (szokásos); a 19 fájl a fő
fában (`mock-alig-var-tanya-*`), a :4600 lead-lapján/`/mock/<id>` belépés után; guest-mobile-check 57 lap-nézet 0 HIBA,
FK-010 19/19 pass; tabló + legjobb/leggyengébb 3 elküldve (★★★ fullbleed, dark-luxury, horizontal, cinematic, parallax ·
...
## Aktív feladat (2026-09-26 — Mobil · Elek a VENDÉG szemével, FK-010)

**📱 A 19 STÍLUS × 2 LEAD MOCKJA TELEFONON, A VENDÉG ÚTJÁN — lokálban mérve és javítva, NEM élesítve.**
Jegyzet: `_planning/memory/2026-09-26_mobil_vendeg_nezet_fk010.md` (wow-ítélet stílusonként, közös vs. sablon-hibák,
előtte/utána képek: `elek/runs/FK-010-kepek/`). A tulaj két pontosítása a szál alatt: a VIZUÁLIS élmény az első
szempont; minden javítás a MOTORBAN (a `mock-*.html` kimenet). Őr: `scripts/guest-mobile-check.mts` (`--selftest`
...
súly) + FK-009 a runnerben (49 lépés, 38 env). **Mérve → javítva (kiszolgáló réteg, a mock-fájlok érintetlenek):**
lábléc-linkek a rögzített réteg alatt 114/114 → `--citui-cfg-clear` (a pirula publikálja a mért réteget, a
lábléc térközt tart); brutalism/parallax viewport-szélesedés (515/390/380 px) → `html,body{overflow-x:clip}` a
/p/ lapon; fekvőn a konfigurátor gombja a süti alatt 38/38 → panel a süti fölött + ≤520 px-en egy oszlopként
görgethető; érintési cél 15 → ≥ 24 px; lazy-load a 3. képtől. **232 → 0 HIBA.** Őr: `lead-mobile-check --gate
--selftest` (pre-commit; 5 sablon-fixture route-interceptorral, 6 szabály, 5 piros kontroll). **Tulaj-döntés
(§2b) vár:** az első képernyő 40–49 %-a chrome (keret-sáv 13–15 % + süti 13–24 % + a sablon Foglalás-sávja
~10 %), a pirula a főcímre ül; **→ testvér-szál/motor:** egyforma fejléc-blokk 19/19, hero-fotó minőség/méret
(2 MB-os pixeles kép), Foglalás-sáv szövegtörés, marquee. ⚠️ Csapda: a `/p/<t>/unsubscribe` GET-re is
leiratkoztat (a mérő az utolsó linken teszi, és `resubscribeProspect`-tel visszaállítja).

## Aktív feladat (legfrissebb szál, 2026-09-25 — a konzol Linear-kerete + navigációs fa)

**🧭 A BELSŐ KONZOL KERETE = A TENANT-ADMIN „LINEAR” NYELVE, EGY BŐVÍTHETŐ FÁBÓL — lokálban kész, NEM élesítve.**
Jegyzet: `_planning/memory/2026-09-25_konzol_linear_keret.md`. Kontraktus: `assets/design-refs/console/linear-shell/`
(README = ami köt, plan.html = a mock, `verify-mock.mjs` 29/29). §2b két kör: A · „Tükör” nyert; a tulaj módosítása:
**a modul-sor maga is lap** (`/hub/crm|finance|report|system`), a fa alatta nyílik, a többi csukva, a nyíl csak hajtogat;
majd „**struktúrát építs** — lesz még számos alrész meg főrész”. Ezért: `src/console/nav.ts` = EGY fa (modul/funkció,
tetszőleges mélység), ebből az oldalsáv, útvonal, ⌘K, telefon-fiók, alsó sáv (első 3 modul + Menü), modul-irányítópult;
`navCounts.ts` a számlálók (15 mp cache, kérés-kontextus, `navCountsOf` tiszta); `HubData` egy adat a kezdőlapnak és a
modul-lapnak. Kivezetve: navy fejléc, gradiens hero, modul-kártyák, `MENU`, `HUB_JS`. CSS: keret + kezdőlap-blokk, `.panel`
Linear-ruhában, `background: --citui-white` → `--citui-panel` (sötét mód). ⚠️ `.con-ib` már létezett (ikon+felirat submit) →
a keret gombja `con-fib`. Őr: `scripts/console-linear-check.mts` (pre-commit, 5 piros kontroll); `help-collapse-check`
szelektora `.is-active`-ra; KB: `console-dashboard` a kódból újraírva, `console-pricing` „hol találod”; `nav.ts` a kb-check
forráslistájában; konzol-súgóképek újralőve (kb-shot). ADR-0233 (a land osztja ki).

## Párhuzamos szál (2026-09-25/26 — rendelés → fizetés, a vevő szemével)

**💳 A RENDELÉS–FIZETÉS ÚT HAZUGSÁGAI ZÁRVA — lokálban kész, landolva, NEM élesítve.**
Jegyzet: `_planning/memory/2026-09-26_rendeles_fizetesi_link.md`. ① Konfigurátor: „Megkaptuk a rendelését”
CSAK `ok:true + pending:true`-ra; 502/hálózati/kezeletlen hiba → őszinte hibasor + újrapróbálás (őr
`order-send-failed-check`). ② Vevő-levelek (`src/email/orderEmail.ts`): visszaigazoló a pay-link nélküli ágon +
a konzol „Fizetési kérés küldése ▸” gombja kiküldi a linket (őr `order-mail-check`). ③ ADR-0112 módosítás:
leiratkozott linken is a TERHELT (kedvezményes) ár látszik, döntés-segítő kártya nélkül (őr
`optout-offer-price-check`). ④ ADR-0234: levélben csak tartós `/pay/go/<fizetés-id>` link — a nyers Barion-link
30 perc után lejárt; kattintáskor frissít, élőre visz / újat indít / kifizetettnél eredmény-lap / már-vásárolt
leadnél „Ezt már megrendelte” (riasztás nélkül); megújítás, felszólítás, domain-lezárás levele is (őr
`pay-entry-check`). Sandbox-fizetés a levélből végig IGAZOLVA (Villa Suzy, 3 420 Ft → tenant + belépés + OV-2026-58).
⚠️ Nyitott: a Villa Suzy siteShot portál-fotója nem töltött be (valószínűleg a rothadó portál-URL jelenség).
...
című tenant-userének (`maintainDatedPrices` → `tenant_user.contact_email`, `.env` smtp, a `CIT_SHOT`
...
## Aktív feladat (legfrissebb szál, 2026-09-25 — az „egész szállás” választható egység)

**🏠 AZ „EGÉSZ SZÁLLÁS” VÁLASZTHATÓ EGYSÉG (ADR-0232, ADR-0114 ①⑦ módosítása) — lokálban kész, NEM
élesítve.** Jegyzet: `_planning/memory/2026-09-25_egesz_szallas_valaszthato.md`. A tulaj bejelentése: az
átnevezett „Apartman 1” beleégve az egész ház (mérve: az `ensureUnits` vissza-jelölte → az Apartman 1
foglalása zárta az Apartman 2-t), szoba nem törölhető, felszereltség-„kavar”. §2b terv a D szoba-szerkesztő
plan.html-jéből származtatva → **B: kártya a rács fölött** („Az egész szállás egyben” — kapcsoló + melyik
egység), a 2. egység felvételekor KÖTELEZŐ kérdés, bármely egység törölhető (utolsó/foglalt nem), ház-tétel
a szobánál is kapcsolható („a ház egészénél is” címke), **az egész saját ára + tulaj-oldali tájékoztató
