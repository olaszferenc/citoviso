...
function loginPlaceholder(res: http.ServerResponse): void {
  send(
    res,
    200,
    `<!DOCTYPE html><html lang="hu"><head><meta charset="utf-8">` +
      `<meta name="viewport" content="width=device-width,initial-scale=1">` +
      `<link rel="stylesheet" href="/assets/ui/citui.css"><title>Bejelentkezés</title></head>` +
      `<body style="background:var(--citui-navy-900)"><div class="citui-container" ` +
      `style="max-width:460px;margin:0 auto;padding:80px 0;text-align:center;color:#fff">` +
      `<h1 style="color:#fff">Bejelentkezés</h1>` +
      `<p style="color:rgba(255,255,255,.7)">A bejelentkezési felület hamarosan elérhető lesz.</p>` +
      `<p><a class="citui-btn citui-btn--primary" href="/">Vissza a főoldalra</a></p></div></body></html>`,
  );
}
...
async function handle(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  if (req.method === "GET") {
    ...
    if (pathname === "/belepes") return loginPlaceholder(res);
