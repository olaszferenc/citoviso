...
export interface SiteTable {
  ...
  /** 0073 (ADR-0208 ⑥.4): when the hourly tick first saw an undeclared price gap;
   *  NULL = none. Cleared when the gap closes (the episode ends). */
  price_gap_since: Timestamp | null;
  /** 0073: when the last weekly price-gap reminder went out (claimed conditionally). */
...
export interface SiteUnitTable {
  ...
  /** 0073 (ADR-0208 ⑥.2): "nem adok meg árat" — where this unit has no price row,
...
export interface UnitPriceTable {
  ...
  /** 0074: the recurring season this row is ONE YEAR's price of. NULL = not a year price. */
  parent_id: string | null;
  /** 0074: which year's occurrence the end-of-season question already went out for. */
