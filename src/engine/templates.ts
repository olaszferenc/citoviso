// Art templates (ADR-0027, template-first). A template is a COMPLETE, reference-fidelity
// page — its own full HTML + CSS at the craft bar of assets/design-refs/reference-quality/ —
// parameterized by SiteData + the recipe's editorial copy. Unlike the composition path
// (primitives arranged by an archetype), the template owns every section's markup, so the
// craft (full-bleed hero, glass booking bar, mosaic gallery, dark review band, rich footer)
// is guaranteed by construction, not assembled from thin shared boxes.
//
// Contracts honored:
//  - Skin/token contract (06-UI-CONTRACT A): all styling dresses from the 11 --cit-* tokens
//    (renderSkinVars) → skins + photo-accent still apply; designCheck token gate passes.
//  - Booking hook (06-UI-CONTRACT B): the enquiry slot is the canonical
//    <section id="cit-enquiry" data-cit-module="booking" data-cit-variant="bar"> — the
//    inline runtime hydrates it into the interactive widget; the no-JS fallback CTA ladder
//    (mailto → tel → disabled) is seeded here, matching the primitives' behavior.
//  - Fact fidelity (§B.17): every data slot renders REAL data only. Sample-capable reviews
//    show the shared marked SAMPLE content in the MOCK phase and are dropped on LIVE.
//    Stats render only real stats. No number, amenity or claim is ever invented here.
//  - mock=live: render(recipe, data) is deterministic; the persisted recipe re-renders the
//    identical page. The LIVE phase may lose places-gated photos (photo policy) → the hero
//    degrades to a token-gradient canvas, everything else stands.
...
export interface ArtTemplate {
   ...
   *  The generator spreads leads across this list (name-hash), killing the monoculture. */
...
function accented(text: string, accent?: string): string {
...
function copyOf(recipe: Recipe, kind: string): SectionCopy {
  return recipe.sections.find((s) => s.kind === kind)?.copy ?? {};
}

/** The canonical booking slot (hydrated by the runtime) with the no-JS CTA ladder. */
function bookingSlot(d: SiteData): string {
...
// ---------------------------------------------------------------------------
// Template: "fullbleed" — the 01-fullbleed-glass reference direction. Full-height
// photo hero under a fixed overlay nav, the glass booking bar floated over the hero
// bottom, dropcap intro with a framed portrait, amenity cards, mosaic gallery, dark
// review band, contact split, rich dark footer, mobile fixed CTA bar.
// ---------------------------------------------------------------------------
...
/** Deterministic skin pick for a template — stable per lead name, spread across the curated
 *  list (djb2 hash). Kills the planner monoculture without randomness (mock=live safe). */
